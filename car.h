...
public:
    ...
    vector<float> speed;
    vector<float> angle;
    vector<float> carEye;               // initialized to EYE_START in init function
    vector<float> carFocus;             // initialized to FOCUS_START in init function
    const float CUBE_CAM_OFFSET = 5;
	const vector<float> EYE_START = {-5.0, 3.0, -5.0};     // -5, 3, 5 and 8, 0, 8 for FOCUS_START seem to be a good "third person" view
